...
public class G extends FrameGrabber {
    ...
    public static FFmpegFrameGrabber createDefault(File deviceFile)   throws FFmpegFrameGrabber.Exception { return new FFmpegFrameGrabber(deviceFile); }
    public static FFmpegFrameGrabber createDefault(String devicePath) throws FFmpegFrameGrabber.Exception { return new FFmpegFrameGrabber(devicePath); }
    public static FFmpegFrameGrabber createDefault(int deviceNumber)  throws FFmpegFrameGrabber.Exception { throw new FFmpegFrameGrabber.Exception(FFmpegFrameGrabber.class + " does not support device numbers."); }
    ...
    private final AVIOInterruptCB.Callback_Pointer cp = new AVIOInterruptCB.Callback_Pointer() {
        ...
        public int call(Pointer pointer) {
            ...
            if (lastFrameTime.get() + FRAME_READ_TIMEOUT < System.currentTimeMillis()) {
                System.out.println("frame read timeout 10s.");
                return 1;
            }
            return 0;
        }
    ...
    private static FFmpegFrameGrabber.Exception loadingException = null;
    public static void tryLoad() throws FFmpegFrameGrabber.Exception {
        if (loadingException != null) {
            ...
            try {
                ...
                if (t instanceof FFmpegFrameGrabber.Exception) {
                    throw loadingException = (FFmpegFrameGrabber.Exception)t;
                } else {
                    throw loadingException = new FFmpegFrameGrabber.Exception("Failed to load " + FFmpegFrameGrabber.class, t);
                }
            }
        }
    }
    ...
    public G(InputStream inputStream, int maximumSize) {
    ...
    public synchronized void releaseUnsafe() throws FFmpegFrameGrabber.Exception {
        started = false;
        ...
        if (inputStream != null) {
            try {
                ...
                throw new FFmpegFrameGrabber.Exception("Error on InputStream.close(): ", ex);
            } finally {
    ...
    @Override public void setFrameNumber(int frameNumber) throws FFmpegFrameGrabber.Exception {
    ...
    @Override public void setTimestamp(long timestamp) throws FFmpegFrameGrabber.Exception {
     ...
     */
    private synchronized void setTimestamp(long timestamp, EnumSet<Frame.Type> frameTypesToSeek) throws FFmpegFrameGrabber.Exception {
        int ret;
        if (oc == null) {
            ...
            if ((ret = avformat_seek_file(oc, -1, Long.MIN_VALUE, early_ts, Long.MAX_VALUE, AVSEEK_FLAG_BACKWARD)) < 0) {
                throw new FFmpegFrameGrabber.Exception("avformat_seek_file() error " + ret + ": Could not seek file to timestamp " + timestamp + ".");
            }
            if (video_c != null) {
    ...
    @Override public void start() throws FFmpegFrameGrabber.Exception {
        start(true);
    }
    ...
    public synchronized void startUnsafe(boolean findStreamInfo) throws FFmpegFrameGrabber.Exception {
        try (PointerScope scope = new PointerScope()) {

            if (oc != null && !oc.isNull()) {
                throw new FFmpegFrameGrabber.Exception("start() has already been called: Call stop() before calling start() again.");
            }
            ...
            if (format != null && format.length() > 0) {
                if ((f = av_find_input_format(format)) == null) {
                    throw new FFmpegFrameGrabber.Exception("av_find_input_format() error: Could not find input format \"" + format + "\".");
                }
            }
            ...
            if ((ret = avformat_open_input(oc, filename, f, options)) < 0) {
                ...
                if ((ret = avformat_open_input(oc, filename, f, options)) < 0) {
                    throw new FFmpegFrameGrabber.Exception("avformat_open_input() error " + ret + ": Could not open input \"" + filename + "\". (Has setFormat() been called?)");
                }
            }
...
//            oc.max_analyze_duration(5*AV_TIME_BASE);
            ...
            if (findStreamInfo && (ret = avformat_find_stream_info(oc, (PointerPointer)null)) < 0) {
                throw new FFmpegFrameGrabber.Exception("avformat_find_stream_info() error " + ret + ": Could not find stream information.");
            }
            ...
            if (video_st == null && audio_st == null) {
                throw new FFmpegFrameGrabber.Exception("Did not find a video or audio stream inside \"" + filename
            ...
            if (video_st != null) {
                ...
                if (codec == null) {
                    throw new FFmpegFrameGrabber.Exception("avcodec_find_decoder() error: Unsupported video format or codec not found: " + video_par.codec_id() + ".");
                }

                /* Allocate a codec context for the decoder */
                if ((video_c = avcodec_alloc_context3(codec)) == null) {
                    throw new FFmpegFrameGrabber.Exception("avcodec_alloc_context3() error: Could not allocate video decoding context.");
                }

                /* copy the stream parameters from the muxer */
                if ((ret = avcodec_parameters_to_context(video_c, video_st.codecpar())) < 0) {
                    releaseUnsafe();
                    throw new FFmpegFrameGrabber.Exception("avcodec_parameters_to_context() error " + ret + ": Could not copy the video stream parameters.");
                }
                ...
                if ((ret = avcodec_open2(video_c, codec, options)) < 0) {
                    throw new FFmpegFrameGrabber.Exception("avcodec_open2() error " + ret + ": Could not open video codec.");
                }
                av_dict_free(options);
                ...
                if ((picture = av_frame_alloc()) == null) {
                    throw new FFmpegFrameGrabber.Exception("av_frame_alloc() error: Could not allocate raw picture frame.");
                }
                if ((picture_rgb = av_frame_alloc()) == null) {
                    throw new FFmpegFrameGrabber.Exception("av_frame_alloc() error: Could not allocate RGB picture frame.");
                }
            ...
            if (audio_st != null) {
                ...
                if (codec == null) {
                    throw new FFmpegFrameGrabber.Exception("avcodec_find_decoder() error: Unsupported audio format or codec not found: " + audio_par.codec_id() + ".");
                }

                /* Allocate a codec context for the decoder */
                if ((audio_c = avcodec_alloc_context3(codec)) == null) {
                    throw new FFmpegFrameGrabber.Exception("avcodec_alloc_context3() error: Could not allocate audio decoding context.");
                }

                /* copy the stream parameters from the muxer */
                if ((ret = avcodec_parameters_to_context(audio_c, audio_st.codecpar())) < 0) {
                    releaseUnsafe();
                    throw new FFmpegFrameGrabber.Exception("avcodec_parameters_to_context() error " + ret + ": Could not copy the audio stream parameters.");
                }
                ...
                if ((ret = avcodec_open2(audio_c, codec, options)) < 0) {
                    throw new FFmpegFrameGrabber.Exception("avcodec_open2() error " + ret + ": Could not open audio codec.");
                }
                av_dict_free(options);

                // Allocate audio samples frame
                if ((samples_frame = av_frame_alloc()) == null) {
                    throw new FFmpegFrameGrabber.Exception("av_frame_alloc() error: Could not allocate audio frame.");
                }
    ...
    @Override public void stop() throws FFmpegFrameGrabber.Exception {
        release();
    }

    @Override public synchronized void trigger() throws FFmpegFrameGrabber.Exception {
        if (oc == null || oc.isNull()) {
            throw new FFmpegFrameGrabber.Exception("Could not trigger: No AVFormatContext. (Has start() been called?)");
        }
        if (pkt.stream_index() != -1) {
            av_packet_unref(pkt);
            pkt.stream_index(-1);
    ...
    private void processImage() throws FFmpegFrameGrabber.Exception {
        ...
        switch (imageMode) {
            ...
            case GRAY:
                // Deinterlace Picture
                if (deinterlace) {
                    throw new FFmpegFrameGrabber.Exception("Cannot deinterlace: Functionality moved to FFmpegFrameFilter.");
                }
                ...
                if (img_convert_ctx == null) {
                    throw new FFmpegFrameGrabber.Exception("sws_getCachedContext() error: Cannot initialize the conversion context.");
                }
    ...
    private void processSamples() throws FFmpegFrameGrabber.Exception {
        int ret;
        ...
        if (audio_c.channels() != getAudioChannels() || audio_c.sample_fmt() != getSampleFormat() || audio_c.sample_rate() != getSampleRate()) {
            if (samples_convert_ctx == null || samples_channels != getAudioChannels() || samples_format != getSampleFormat() || samples_rate != getSampleRate()) {
                ...
                if (samples_convert_ctx == null) {
                    throw new FFmpegFrameGrabber.Exception("swr_alloc_set_opts() error: Cannot allocate the conversion context.");
                } else if ((ret = swr_init(samples_convert_ctx)) < 0) {
                    throw new FFmpegFrameGrabber.Exception("swr_init() error " + ret + ": Cannot initialize the conversion context.");
                }
            ...
            if ((ret = swr_convert(samples_convert_ctx, plane_ptr.put(samples_ptr_out), sample_size_out, plane_ptr2.put(samples_ptr), sample_size_in)) < 0) {
                throw new FFmpegFrameGrabber.Exception("swr_convert() error " + ret + ": Cannot convert audio samples.");
            }
            for (int i = 0; i < planes_out; i++) {
    ...
    public synchronized Frame grabFrame(boolean doAudio, boolean doVideo, boolean doProcessing, boolean keyFrames, boolean doData) throws FFmpegFrameGrabber.Exception {
        try (PointerScope scope = new PointerScope()) {

            if (oc == null || oc.isNull()) {
                throw new FFmpegFrameGrabber.Exception("Could not grab: No AVFormatContext. (Has start() been called?)");
            } else if ((!doVideo || video_st == null) && (!doAudio || audio_st == null)) {
                return null;
            }
            if (!started) {
                throw new FFmpegFrameGrabber.Exception("start() was not called successfully!");
            }
            ...
            while (!done &&(System.currentTimeMillis() - lastFrameTime.get() < 5000)) {
                int ret = 0;
                if (readPacket) {
                    if (pkt.stream_index() != -1) {
                ...
                if (doVideo && video_st != null && pkt.stream_index() == video_st.index()
                    ...
                    while (!done) {
                        ...
                        if (ret == AVERROR_EAGAIN() || ret == AVERROR_EOF()) {
                            ...
                            throw new FFmpegFrameGrabber.Exception("avcodec_receive_frame() error " + ret + ": Error during video decoding.");
                        }
                        got_frame[0] = 1;
                    ...
                    if (readPacket) {
                        ...
                        if (ret < 0) {
                            throw new FFmpegFrameGrabber.Exception("avcodec_send_packet() error " + ret + ": Error sending an audio packet for decoding.");
                        }
                    }
                    ...
                    while (!done) {
                        ...
                        if (ret == AVERROR_EAGAIN() || ret == AVERROR_EOF()) {
                            ...
                            throw new FFmpegFrameGrabber.Exception("avcodec_receive_frame() error " + ret + ": Error during audio decoding.");
                        }
                        got_frame[0] = 1;
    ...
    public synchronized AVPacket grabPacket() throws FFmpegFrameGrabber.Exception {
        if (oc == null || oc.isNull()) {
            throw new FFmpegFrameGrabber.Exception("Could not grab: No AVFormatContext. (Has start() been called?)");
        }
        if (!started) {
